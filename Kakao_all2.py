...
def user_following_list_func(user_id):
    tmp2 = users[users.id==user_id].following_list
    tmp2 = tmp2.tolist()
    tmp2 = flatten(tmp2)
    return tmp2

def recent_same_readfollow(recent1_metadata , read , user_following_list):
...
def all_read_similar(read , metadata , user_id , model):
    ...
    similar_read_data = sorted( score , key = (lambda x:x[1]) , reverse=True)[:10]
    ttmp =[]
    for i in similar_read_data:
        ttmp.append(i[0])    
    return metadata.iloc[ttmp,:].id
